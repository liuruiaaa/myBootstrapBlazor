...
using Switch = BootstrapBlazor.Components.Switch;


namespace BootstrapBlazor.Maui.Models;
...
public class Foo
{
    ...
    public static Foo Generate(IStringLocalizer<Foo> localizer) => new()
    {
        ...
        Education = random.Next(1, 100) > 50 ? EnumEducation.Primary : EnumEducation.Meddle
    };
    ...
    /// </summary>
    /// <returns></returns>
    public static List<Foo> GenerateFoo( int count = 80)
        => Enumerable.Range(1, count).Select(i => new Foo()
        {
            Id = i,
            Name = "Foo.Name"+$"{i:d4}",
            DateTime = DateTime.Now.AddDays(i - 1),
            Address = "lFoo.Address"+$"{random.Next(1000, 2000)}",
            Count = random.Next(1, 100),
            Complete = random.Next(1, 100) > 50,
            Education = random.Next(1, 100) > 50 ? EnumEducation.Primary : EnumEducation.Meddle
        }).ToList();
    public static List<Foo> GenerateFoo(IStringLocalizer<Foo> Localizer,int count = 80)
    => Enumerable.Range(1, count).Select(i => new Foo()
    {
        Id = i,
        Name = "Foo.Name" + $"{i:d4}",
